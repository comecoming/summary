...
int main(int argc, char* argv[])
{
   ...
   char *errmsg = NULL;
	...
	if (sqlite3_exec(db, "create table if not exists mytbl (id int, name text, pwr int, agi int, magic int)", callback, "create mytbl", &errmsg))
		printf("%s", sqlite3_errmsg(db));
	...
	//if (SQLITE_OK != sqlite3_exec(db, "delete from mytbl where id=2", callback, "select mytbl", &errmsg))
	//	printf("%s", sqlite3_errmsg(db));
